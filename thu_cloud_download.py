...
def parse_args():
    ...
    args.add_argument('-x', '--max', type=float, default=None, help='Maximum file size to be downloaded (MB)')
    args.add_argument('-n', '--min', type=float, default=0, help='Minimum file size to be downloaded (MB)')
...
def main():
    args = parse_args()
    url, pattern, save_dir, max_size, min_size = args.link, args.file, args.save_dir, args.max, args.min
    ...
    for file in filelist:
        if not max_size is None:
            if max_size >= file["size"] / 1024 / 1024 >= min_size:
                ...
                total_size += file["size"]  # MB
        elif file["size"] / 1024 / 1024 >= min_size:
            target_files.append(file)
            total_size += file["size"]  # MB
    print_filelist(target_files)
    logging.info(f"# Files: {len(target_files)}. Total size: {total_size / 1024 / 1024: .1f} MB.")
    ...
    download(share_key, target_files, save_dir)
